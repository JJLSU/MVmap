...
window.addEstateBake=map=>{const url=(z,x,y)=>window.ESTATE_TILES?.[z+'/'+x+'/'+y]||'baked/tiles/'+z+'/'+x+'/'+y+'.webp?v='+encodeURIComponent(m.tile_revision||'09');
...
const Tile=L.GridLayer.extend({createTile(coords,done){const c=document.createElement('canvas');c.width=c.height=512;c.setAttribute('role','presentation');const ctx=c.getContext('2d');
 const load=(z,x,y,sx,sy,sw)=>{const img=new Image();img.onload=()=>{ctx.drawImage(img,sx,sy,sw,sw,0,0,512,512);done(null,c);};img.onerror=()=>{if(z>14){const px=Math.floor(x/2),py=Math.floor(y/2),q=256*Math.pow(2,-(coords.z-z+1)),qx=(x-2*px),qy=(y-2*py);load(z-1,px,py,sx/2+qx*256,sy/2+qy*256,sw/2);}else done(null,c);};img.src=url(z,x,y);};
